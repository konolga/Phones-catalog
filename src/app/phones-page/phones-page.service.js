...
export class PhonesPageService {
  ...
  async  getAllPhones({ text, orderBy }) {
   ...
   // return new Promise((res) => {
    const phones = await fetch(`https://konolga.github.io/Phones-store/src/phones/phones.json`)
      .then((res) => res.json());
  ...
  async getPhonesById(id) {
    try{
    return await fetch(`https://konolga.github.io/Phones-store/src/phones/${id}.json`)
    .then((res) => res.json());
    }
    catch (err) {
      console.log(err);
